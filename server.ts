...
app.get('/api/:documentType/:id/pdf', async (req, res) => {
    ...
    const { startDate, endDate } = req.query; // Only relevant for statements

    // Initialize doc outside try/catch to ensure it's always available for doc.end()
    ...
    try {
        switch (documentType) {
            ...
            case 'invoice': {
                ...
                if (invoiceQueryResult.rows.length === 0) {
                    // Send JSON error and immediately return if invoice not found
                    res.status(404).json({ error: 'Invoice not found' });
                    doc.end(); // Ensure PDF stream is ended even if not piped
                    return; // Crucial: stop execution
                }

                const invoice = invoiceQueryResult.rows[0];

                // Set response headers ONLY AFTER confirming invoice exists
                ...
                doc.lineWidth(0.5).strokeColor('#cccccc').moveTo(col1X, tableTop + 15).lineTo(550, tableTop + 15).stroke(); // Underline headers
                ...
                invoice.line_items.forEach((item: any) => {
                    if (yPos + 20 > doc.page.height - doc.page.margins.bottom) {
                        doc.addPage();
                        yPos = doc.page.margins.top; // Reset yPos for new page
                        // Re-draw headers on new page if desired
                        doc.fontSize(10)
                            .font('Helvetica-Bold')
                ...
                doc.moveDown();
                doc.lineWidth(0.5).strokeColor('#cccccc').moveTo(col1X, yPos).lineTo(550, yPos).stroke(); // Underline total
                ...
                return; // Stop further execution in this route handler
            }

            case 'statement': {
                // Your existing statement generation logic would go here
                // Make sure it also calls doc.end() when done and returns.
                res.setHeader('Content-Type', 'application/pdf'); // Set headers for statement
                doc.pipe(res);
            ...
            default:
                // For unsupported types, send JSON error and return immediately
        ...
        // If they have, it means doc.pipe(res) started sending the PDF
        // and then an error occurred during PDF content generation.
        // In this case, we cannot send a JSON error.
        if (res.headersSent) {
            console.error('Headers already sent. Cannot send JSON error for PDF generation error.');
            // Just end the PDF stream gracefully if possible,
            // or let the connection close by itself if it's already broken.
            doc.end();
        ...
        doc.end(); // Always ensure doc.end() is called even on error path
        return; // Important to return
    }
});
